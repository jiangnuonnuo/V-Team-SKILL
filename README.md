...
`v-team` 是面向自用或小团队的 Codex/Claude 多 Agent 项目协作技能。它在单个项目目录内生成根约束、个人身份、模块白名单、单份活动计划和临时对接索引，并提供提交前范围检查与对接文档清理。
...
- 每次本地提交以可用的完整功能或明确修复为单位，可使用 `feat`、`fix` 等语义类别，不按文件机械拆分。
- `check-plan` 校验固定 review 记录，语义 review 仍由独立 Agent 完成。
- 本地提交前只运行一次 `git diff --cached --name-only` 范围检查。
- 越界修改由用户授权当前提交，不永久扩展白名单。
- 对接资料只在 `Plan/collaboration/active/` 中临时保存；协作时优先由 `handoffs.md` 索引，但仍可读取代码和已有资料。
- `Plan/` 通过本地 Git 排除和提交检查避免进入提交；关闭 handoff 后删除其临时对接文档，不归档。
- 完成或废弃计划会验证已完成任务的本地提交真实存在并重置活动计划。
...
python "F:\java\code\SKILLS\V-Team-SKILL\scripts\vteam.py" agent --project-root "F:\java\code\item\example-project" --agent-id backend-1 --runtime codex --role backend --responsibility "用户与权限后端" --scope "用户与权限后端全部代码" --module backend/auth --allow backend/auth/ --allow tests/auth/ --read-doc Plan/collaboration/handoffs.md
...
python "/Users/me/skills/V-Team-SKILL/scripts/vteam.py" agent --project-root "/Users/me/projects/example-project" --agent-id frontend-1 --runtime claude --role frontend --responsibility "登录页面与接口集成" --scope "用户端前端全部代码" --module frontend/auth --allow frontend/auth/ --read-doc Plan/collaboration/handoffs.md
